...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-89%20passed-22C55E" alt="Tests" />
</p>
...
검색창에 **"삼성 공기청정기 필터"**처럼 평소 말하듯 입력하면 일치하는 제품이 사진·출처 주소와 함께 바로 나타납니다. 이름을 모르면 **제품 전체 사진, 고장 난 부위, 라벨 사진**을 올리세요. AI가 이미지에서 확인 가능한 정보를 구조화해 읽고, 모델명이 보이지 않아도 제품 종류·브랜드 로고·코드 조각으로 **가장 유력한 후보를 목록 맨 위에** 올립니다.

사용자는 후보 중에서 자신의 제품을 고른 뒤, 해당 모델에 맞는 **부품 정보·호환 근거·구매 경로**를 한 화면에 하나씩 확인합니다.

> **핵심 원칙**  
> 라벨에서 읽은 것(확정 후보)과 디자인으로 짐작한 것(추천 후보)을 **절대 섞지 않고**, **재고와 호환성을 분리**하며, 근거가 부족한 경우에는 명확하게 **“확인 필요”**로 안내합니다. 부품이 맞는지는 AI가 아니라 출처가 있는 자료로만 판단합니다.

| 제품 찾기 · 검색하면 사진과 주소가 함께 | 부품 선택 · 맞는 부품과 판정 |
| --- | --- |
| ![딱품 첫 화면](./artifacts/ui-journey/home-search-1440.png) | ![딱품 부품 선택 화면](./artifacts/ui-journey/parts-1440.png) |
...
| 기능 | 설명 |
| --- | --- |
| **검색으로 찾기** | "삼성 공기청정기 필터"처럼 제품 종류·브랜드·모델·부품을 섞어 입력하면 제품 사진과 출처 주소가 담긴 제안 목록이 뜹니다. 한글·영문 별칭, 공백, 하이픈, 전각 문자를 정규화합니다. |
| **사진으로 찾기** | 전체·부품·라벨 사진을 최대 4장까지 분석하고, EXIF 제거·회전·축소를 서버에서 처리합니다. 찾는 부품을 함께 적으면 결과가 그 부품으로 바로 이어집니다. |
| **사진 기반 추천** | 모델 코드를 읽지 못해도 AI가 디자인으로 짐작한 브랜드·제품군(`visualHints`)과 제품 종류·로고·코드 조각으로 카탈로그를 점수화해 유력한 제품을 맨 위에 올립니다. 라벨에서 읽은 `모델명 일치`와 외형 기반 `사진 기반 추천`은 따로 표시하고, 자동 선택하지 않습니다. AI의 추정("LG 퓨리케어 에어로타워")은 웹 검색 링크와 함께 보여 줍니다. |
| **한 화면에 결정 하나** | 제품 찾기 → 제품 확인 → 부품 선택 → 구매처 확인. 화면마다 뒤로 가기가 한 단계씩 동작합니다. |
| **등록 제품 둘러보기** | 물건 종류·브랜드 기준으로 카탈로그를 좁혀 봅니다. |
...
```mermaid
flowchart LR
    A[1 제품 찾기<br/>검색 또는 사진] --> B[AI 이미지 분석 / OCR]
    A --> C
    B --> C[2 제품 확인<br/>유력한 후보가 맨 위]
    C --> D[3 부품 선택<br/>판정과 함께]
    D --> E[4 구매처 확인<br/>살 곳 → 호환 근거]
    D --> F[다른 방법으로 찾기<br/>검색·중고·제조사 문의]
```

1. 검색어 또는 사진으로 탐색을 시작합니다. 정확한 모델 코드를 입력하거나 제안 목록에서 고르면 바로 부품 화면으로 갑니다.
2. AI는 관찰 기록 도구 하나만 호출하며 자유 텍스트 응답은 사용하지 않습니다. 외형으로 짐작한 브랜드·제품군은 관찰과 분리된 `visualHints`에만 적습니다.
