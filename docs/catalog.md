...
이전 체크포인트: **제품 52개, 부품 46개, 판매 46개, 적용·제외 근거 101개**, 제품 브랜드 5개다. 1차 확장 이후 킨토 17개, 조지루시 15개 모델과 부품 24개를 추가했다. 국내 판매 경로를 가진 모델은 48개이며 해당 표시가 재고 확보나 실제 장착 검증을 의미하지 않는다.
...
현재 숫자와 분야별 근거는 [생활 전반 확장](daily-life-expansion.md)을 따른다. 위 수치는 이전 확장 단계의 이력이다.

- `products.group`은 분야이며 기존 자료의 기본값은 `drinkware`다. 신규 제품은 명시적으로 지정한다.
- `GET /api/catalog?group=electronics&category=remote&domestic=true`처럼 분야와 필요한 부품을 함께 필터링한다. 필터 상품을 가진 모델이 리모컨 구매 경로까지 가진 것으로 잘못 표시되지 않는다.
- 공급자 허용 카테고리와 서버 확인 항목은 `shared/taxonomy.ts` 및 `shared/domain.ts`에서 관리한다. `other`는 특정 부품이 분류에 없을 때 이름·규격을 직접 입력하는 경로다.
- 학용품 리필, 재봉틀 보빈, 가구 선반은 같은 이름·외경만으로 연결하지 않는다. 본체 코드·원형/사각형·세대·실감기 축 등 예외 조건을 보존한다.
- 브랜드가 다른 부품 판매처의 적용 주장은 `seller`로 기록한다. SINGER용 대체 보빈은 `aftermarket`이며 실제 장착 테스트로 승격하지 않는다.
- 신규 링크 검사 기록은 `artifacts/catalog-daily-links.json`. 403·시간 초과는 접근 제한으로 표시하고 단종·품절로 해석하지 않는다.
